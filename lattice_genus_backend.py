...
from sage.all import (ZZ, QQ, matrix, prod, QuadraticForm, BinaryQF,
                      next_prime)
...
def discriminant_input(signature, orders, entries):
    if len(signature) != 2 or any(type(x) is not int or x < 1 for x in signature):
        raise ValueError("signature must be [positive_rank, negative_rank], both positive")
    if any(type(d) is not int or d < 2 for d in orders):
