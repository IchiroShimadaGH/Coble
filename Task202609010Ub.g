...
GensOGSize:=function(tGram, gens, trec)
  local vs, tg, tperm, gensperms, permsize, G;
  ...
  od;
  G:=Group(gensperms);
  trec.structure:=StructureDescription(G);
  Printn(trec.structure);
  permsize:=Size(G);
  return(permsize);
end;
...
for trec in Rrecs do 
  ...
  if trec.ogsize<>GensOGSize(trec.Gram, trec.gens) then 
    ...
    Printn("ogsize check", pos, trec.ogsize, Collected(trec.ADEtype));
  fi;
od;

savedataas(Rrecs, "RRrecs");
