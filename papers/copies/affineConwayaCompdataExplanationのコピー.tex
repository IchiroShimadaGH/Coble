...
%\item  \tname{OrbitDecompbarDelta.txt} (7.4 MB), 
%\item  \tname{OrbitDecompDelta.txt}  (5.3 MB), and 
%\item  \tname{F19model.txt} (2 KB), 
\end{itemize}
...
Some parts of the data are formatted using the {\tt Record} format of {\tt GAP}.
...
\infty, 0, 1,2 , \dots, 22, 
\]
and we  assign an order to the standard  basis of 
$\FF_2^{\Omega}\cong \FF_2^{24}$ or of $\ZZ^{\Omega}\cong \ZZ^{24}$
accordingly.
Hence, 
...
This data \tname{LeechBasis}  is a list of $24$ vectors in $\ZZ^{\Omega}$.
%
\item \tname{LeechGram} is the Gram matrix of  $\Lambda$
with respect to \tname{LeechBasis}. We have
\[
\mathtt{LeechGram} =\frac{1}{8}\cdot \mathtt{LeechBasis}\cdot{}^{t} \mathtt{LeechBasis}.
\]
%
\item \tname{epsilonrecs} is the list of $4$ records,
each of which describes the involution $\vep_n=\vep(C_n)\in \dotz$
for $n=8, 12, 16, 24$.
%(Caution: not epsilonrecs,  but epsilonrecs.)
...
%
\item \tname{Cvect} is the codeword $C_n\in \Golay(n)$ such that $\vep_n=\vep(C)$,
which is an element of \tname{GolayVects}.
%
\item \tname{Cword} is the  element of \tname{GolayWords} corresponding to \tname{Cvect}.
%
\item \tname{mat} is the matrix representation of $\vep_n\in \dotz$
...
%
\item \tname{plusOGrec}  is the record describing 
$\OG(\Leech(\vep_n, +))$ by the format explained in the next section,
with the initial basis $[e_1, \dots, e_m]$ is given by  \tname{plusbasis}.
%
...
%
\item \tname{minusOGrec}  is the record describing 
$\OG(\Leech(\vep_n, -))$ by the format explained in the next section,
with the initial basis $[e_1, \dots, e_m]$ is given by  \tname{minusbasis}.
%
\item \tname{piepsilon} is  the matrix representation of
$\pi_{\vep_n}\colon \Leech \to \Leech(\vep_n, -)$
with respect to the bases  \tname{LeechBasis} of $\Leech$
and \tname{minusbasis} of $\Leech(\vep_n, -)$.
 \end{itemize}
%
