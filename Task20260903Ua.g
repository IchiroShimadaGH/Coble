...
Printn("walls", List(nSaSvSss0, xx->Length(xx.vSliftss)));

savedata(nSaSvSss0);

GetGeneralPeriodcond:=function(discg)
  local leng, ii, qgs, ones;
  leng:=Length(discg);
  ones:=List([1..leng], ii->1);
  qgs:=Set([DiagonalMat(ones), DiagonalMat(discg-ones)]);
  return(qgs);
end;

Borrec.priodcond:=GetGeneralPeriodcond(Borrec.discS.discg);
savedata(Borrec);
...
autchamrec0:=AutCham(Borrec, nSaSvSss0);

Printn("nonperiodaut", Length(autchamrec0.nonperiodaut), 
"periodaut", Length(autchamrec0.periodaut));

Printn("nonperiodorbs", Length(autchamrec0.nonperiodorbs), 
"periodorbs", Length(autchamrec0.periodorbs));

savedata(autchamrec0);
