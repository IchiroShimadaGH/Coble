FindTempMinNormBasis:=function(GramL, trialnumb)
  #
  local nn, norm, tU, tt,GramL2,  LLrec, basis, ii,  
  basisnrmslist,  maxnrm,  basisrec;
  #
  nn:=Length(GramL);
  if SignatureQ(GramL)<>[nn, nn, 0] then beep(665522); fi;
  norm:=infinity;
  for tt in [1..trialnunb] do
    tU:=RandomUnimodMat(nn);
    GramL2:=TMTTmult(tU, GramL);
    LLrec:=LLLReducedGramMat(GramL2);
    basis:=LLLrec.transformation;
    #newGram:=LLLrec.remainder;
    #if newGram<>TMTTmult(basis, GramL2) then beep(11211); fi;
    basisnrmslist:=List([1..nn], ii->LLLrec.remainder[ii][ii]);
  ...
  od;
  if norm=infinity then beep(8181818); fi;
  basisrec:=rec(
    norm:=norm, 
    basis:=minbasis
  );
...
AreIsomPosLats:=function(GramL1, GramL2)
  ...
  beep:=function(beepnumb)
    localbeep("AreIsomPosLats", beepnumb); Error();
  end;
  #
 
  nn:=Length(GramL1);
  if nn<>Length(GramL2) then return(false); fi;
  det1:=DeterminantIntMat(GramL1);
  det2:=DeterminantIntMat(GramL2);
  if det1<>det2 then return(false); fi;
  basisrec1:=FindTempMinNormBasis(Gram1, 10);
  basisrec2:=FindTempMinNormBasis(Gram2, 10);
